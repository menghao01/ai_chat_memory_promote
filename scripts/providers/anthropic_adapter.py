...
class AnthropicProvider(BaseProvider):
    def __init__(self, default_model: str = "claude-3-5-sonnet-latest", transport: Optional[Transport] = None):
        ...
        if not self.transport:
            ...
            return ConversationResponse(
                ...
                error=ProviderError(
                    ...
                    message="No transport configured for Anthropic provider.",
                    provider="anthropic",
                    retryable=False,
                ),
            )

        payload, transport_error = execute_transport("anthropic", request, self.transport)
        latency = int((time.perf_counter() - started) * 1000)
        if transport_error:
            return ConversationResponse(
                provider="anthropic",
