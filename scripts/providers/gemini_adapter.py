...
class GeminiProvider(BaseProvider):
    def __init__(self, default_model: str = "gemini-2.0-flash", transport: Optional[Transport] = None):
        ...
        if not self.transport:
            ...
            return ConversationResponse(
                ...
                error=ProviderError(
                    ...
                    message="No transport configured for Gemini provider.",
                    provider="gemini",
                    retryable=False,
                ),
            )

        payload, transport_error = execute_transport("gemini", request, self.transport)
        latency = int((time.perf_counter() - started) * 1000)
        if transport_error:
            return ConversationResponse(
                provider="gemini",
